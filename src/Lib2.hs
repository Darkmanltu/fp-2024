...
parseQuery :: String -> Either String (String, Query)
parseQuery input =
      ...
      case parseWord rest of
        Right ("BuyBundle", rest1) ->
          case parseBundle rest1 of
            Right (bundle, rest2) -> Right (rest2, BuyBundle bundle) -- Handle BuyBundle directly
            Left err -> Left $ "Failed to parse bundle: " ++ err
        Right ("Buy", rest1) ->
          case parseItem rest1 of
            Right (item, rest2) -> Right (rest2, Buy item)
            Left err -> Left $ "Failed to parse Buy command: " ++ err
        Right ("Sell", rest1) ->
          case parseItem rest1 of
            Right (item, rest2) -> Right (rest2, Sell item)
            Left err -> Left $ "Failed to parse Sell command: " ++ err
        Right ("ViewInventory", rest1) -> Right (rest1, ViewInventory)
        Right ("ResetToStarterKit", rest1) ->
          case parseWord rest1 of
            Right ("starterKit", rest2) -> Right (rest2, ResetToStarterKit starterKit)
            Right ("advancedkit", rest2) -> Right (rest2, ResetToStarterKit advancedkit)
            Right ("proKit", rest2) -> Right (rest2, ResetToStarterKit proKit)
